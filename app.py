...
WHO_LIMITS = {
    "pH": (6.5, 8.5),
    "TDS_ppm": (0, 500),
    # Informational only: WHO's actual NTU threshold. Our sensor reports a
    # binary Clear/Turbid state (see _turbidity_flag), not a raw NTU number,
    # so this range is not used for a numeric compliance check on turbidity.
    "Turbidity_NTU": (0, 5),
    "Temperature_C": (20, 30),
}
...
def ml_prediction(ph, tds, turbidity, temperature):
    if scaler is None or model is None:
        raise RuntimeError("The ML model is not available.")

    turbidity_flag = _turbidity_flag(turbidity)
    new_sample = pd.DataFrame(
        [[ph, tds, turbidity_flag, temperature]],
...
def _turbidity_flag(turbidity):
    """The turbidity sensor reports its reading directly as a binary state
    (0 = clear, 1 = turbid) — the same encoding used to train the model —
    so this just normalizes the value rather than applying any threshold."""
    return int(round(float(turbidity)))


def _turbidity_label(turbidity):
    """Human-readable label for the binary turbidity state."""
    return "Turbid" if _turbidity_flag(turbidity) else "Clear"
...
def _score_breakdown(ph, tds, turbidity, temperature):
    ...
    turb_score = 0 if _turbidity_flag(turbidity) else 100
...
def _format_reading(val):
    """Reading values are numeric for pH/TDS/Temperature but a Clear/Turbid
    string for turbidity, so only apply numeric formatting when it applies."""
    return f"{val:g}" if isinstance(val, (int, float)) else str(val)


def _who_compliance(col, val):
    """Returns (reading_display, range_display, meets) for the WHO
    compliance table. Turbidity is a special case: the sensor only reports
    a binary Clear/Turbid state, not a raw NTU value, so there's no numeric
    range to check it against — only whether it's Clear."""
    if col == "Turbidity_NTU":
        meets = _turbidity_flag(val) == 0
        return _turbidity_label(val), "Clear", meets
    low, high = WHO_LIMITS[col]
    meets = low <= val <= high
    return val, f"{low} – {high}", meets
...
def _param_value_for_reference_comparison(col, val):
    """The reference/training dataset stores Turbidity_NTU as a binary flag
    (see load_cleaned_dataset), so comparisons against it (histogram marker,
    percentile) need the reading converted to that same 0/1 encoding."""
    if col == "Turbidity_NTU":
        return _turbidity_flag(val)
    return val
...
def _build_distribution_fig(ph, tds, turbidity, temperature, white_bg=False):
    params = [
        ("pH", ph, "pH"),
        ("TDS_ppm", tds, "TDS (ppm)"),
        ("Turbidity_NTU", turbidity, "Turbidity (Clear = 0 / Turbid = 1)"),
    ...
    for ax, (col, val, label) in zip(axes.flatten(), params):
        ...
        if col == "Turbidity_NTU":
            marker_val = _turbidity_flag(val)
            legend_label = f"Your reading: {_turbidity_label(val)}"
        else:
            marker_val = val
            legend_label = f"Your reading: {val}"

        ax.axvline(marker_val, color=CHART_MARK, linewidth=2, label=legend_label)
        if col == "Turbidity_NTU":
            # Binary axis: shade only the "Clear" (0) state as the safe zone,
            # rather than a numeric NTU range.
            ax.axvspan(-0.5, 0.5, color=CHART_SAFE_BAND, alpha=0.15)
            ax.set_xticks([0, 1])
            ax.set_xticklabels(["Clear", "Turbid"])
        else:
            low, high = WHO_LIMITS[col]
...
def build_eda_print_assets(ph, tds, turbidity, temperature, purity):
    ...
    for col, val, label in params:
        reading, range_display, meets = _who_compliance(col, val)
        who_rows.append({
            "Parameter": label, "Reading": reading,
            "WHO Range": range_display,
            "Status": "Within range" if meets else "Outside range",
        })
    assets["who_rows"] = who_rows
    assets["percentiles"] = {
        label: _percentile_in_dataset(reference_df, col, _param_value_for_reference_comparison(col, val))
        for col, val, label in params
...
def render_eda(ph, tds, turbidity, temperature, purity):
    ...
    with tab1:
        ...
        for c, (col, val, label) in zip(cols, params):
            pct = _percentile_in_dataset(reference_df, col, _param_value_for_reference_comparison(col, val))
    ...
    with tab4:
        ...
        table_rows = "".join(
            ...
            f"<td data-label='Reading'>{_format_reading(r['Reading'])}</td>"
        ...
        card_rows = "".join(
            ...
            f"<div class='who-card-row'><span class='who-card-label'>Reading</span><span class='who-card-value'>{_format_reading(r['Reading'])}</span></div>"
...
def render_landing():
    ...
    <div class="detail-card">
        ...
        that gap with a low-cost, data-driven way to screen water quality and flag
        what to do next if a reading looks concerning.</p>
        <p><strong>What FlowGuard actually assesses:</strong> the system evaluates the
        physical and basic physicochemical quality of water using pH, turbidity, TDS,
        and temperature. The results provide an indication of overall water quality but
        do not replace comprehensive laboratory testing required to confirm
        microbiological and chemical safety for drinking. The Purity Score / Water
        Quality Index is an indicator of overall quality, not a certification that the
        water is safe for human consumption.</p>
        <p><strong>Where it can be used:</strong> tap water, borehole water, well water,
        river water, rainwater, and reservoir or storage tank water — helping identify
        water that may need further treatment or lab testing before it's confirmed
        safe to drink.</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_b:
        ...
        <div class="detail-card">
            ...
            <table class="param-table">
                ...
                <tr><td><b>Turbidity (Clear / Turbid)</b></td><td>Water clarity / cloudiness</td></tr>
    ...
    if total_tests == 0:
        ...
        for _, row in recent.iterrows():
            ...
            <div class="record-row">
                ...
                <div class="record-sub">pH {row['pH']} · TDS {row['TDS_ppm']} ppm · Turbidity {_turbidity_label(row['Turbidity_NTU'])} · {row['Temperature_C']}°C</div>
            </div>
            """, unsafe_allow_html=True)
...
def render_new_test():
    ...
    with st.form("test_form"):
        st.markdown('<div class="new-test-panel">', unsafe_allow_html=True)
        ...
        with col1:
            ph = st.number_input(
                ...
                step=0.01,
                format="%.2f",
                help="Measure the water's acidity or alkalinity.",
            )
            tds = st.number_input(
                "TDS (ppm)",
                min_value=0.0,
        ...
        with col2:
            turbidity = st.number_input(
                ...
                value=0,
                step=1,
                help="Sensor reading: 0 = Clear, 1 = Turbid.",
            )
...
def render_result_block(dt_str, ph, tds, turbidity, temperature,
    ...
    ph_meets = _who_compliance("pH", ph)[2]
    tds_meets = _who_compliance("TDS_ppm", tds)[2]
    turb_meets = _who_compliance("Turbidity_NTU", turbidity)[2]
    temp_meets = _who_compliance("Temperature_C", temperature)[2]

    def metric_box(accent_class, icon, label, value, meets):
        badge_class = "badge-safe" if meets else "badge-unsafe"
        badge_text = "Within range" if meets else "Outside range"
        return (
            f"<div class='metric-box {accent_class}'>"
            f"<div class='metric-top'><span class='metric-icon'>{icon}</span>"
            f"<span class='metric-label'>{label}</span></div>"
            f"<div class='metric-value'>{value}</div>"
            f"<span class='badge {badge_class} metric-badge'>{badge_text}</span>"
            f"</div>"
        )

    metrics_html = (
        metric_box("metric-ph", "🧪", "pH", ph, ph_meets)
        + metric_box("metric-tds", "💧", "TDS", f"{tds} ppm", tds_meets)
        + metric_box("metric-turbidity", "🌫️", "Turbidity", _turbidity_label(turbidity), turb_meets)
        + metric_box("metric-temp", "🌡️", "Temperature", f"{temperature} °C", temp_meets)
    )
    ...
    <div class="container">
        ...
        <div class="readings-analysis-grid">
            <div class="card">
                ...
                <p><strong>Turbidity:</strong> {_turbidity_label(turbidity)}
                    <span style="color:#7c98a6;font-size:12px;">(sensor code: 0 = Clear, 1 = Turbid)</span></p>
...
def render_print_window_button(label: str, unique_key: str, html_content: str):
    ...
    components.html(f"""
        ...
        <button id="{safe_id}" style="
            background:linear-gradient(135deg, #10638a 0%, #2eb8c9 100%);color:white;border:1px solid rgba(46,184,201,0.35);
            padding:10px 18px;height:48px;min-height:48px;box-sizing:border-box;border-radius:10px;cursor:pointer;
            font-size:14px;font-weight:600;white-space:nowrap;line-height:1.2;box-shadow:0 10px 26px rgba(0,0,0,0.35);width:100%;">
            {label}
        </button>
        <script>
...
def build_history_print_html(df: pd.DataFrame) -> str:
    ...
    rows_html = "".join(
        ...
        f"<td>{_turbidity_label(r['Turbidity_NTU'])}</td><td>{r['Temperature_C']}</td>"
    ...
    <body><div class="container">
        ...
        <table>
            <tr><th>Date/Time</th><th>pH</th><th>TDS (ppm)</th><th>Turbidity</th>
...
def render_history():
    ...
    for idx, row in df_display.iterrows():
        with st.container(key=f"history_row_{idx}"):
            ...
            cols[3].markdown(f"Turb: {_turbidity_label(row['Turbidity_NTU'])}", unsafe_allow_html=True)
